...
void Pantalla_DibujaLineaDesdeImagen(Imagen img, int sx, int x, int y, int h) {
    int from = y < 0 ? 0 : y;
    int to = y + h >= pantalla.height ? pantalla.height - 1 : y + h;

    if (sx < 0 || sx >= img->surface->w) return;

    for (int i = from; i <= to; i++) {
        double ni = (double) ( i - y) / (h);
        int sy = ni * img->surface->h;
        if (sy < 0 || sy >= img->surface->h) continue;
        char* pixel_fuente = img->surface->pixels + sy * img->surface->pitch + sx * 4;
        char* pixel_destino = pantalla.surface->pixels + i * pantalla.surface->pitch + x * 4;

        memcpy(pixel_destino, pixel_fuente, 4);
    }
}
